...
var renderInteractiveJSON = function renderInteractiveJSON($target, parentKey, offset, top) {
  ...
  // keeping these for now because line numbers depend on them
  ...
  var addNewBracket = function(bracketString, bracketId, parentKey, $parentDiv) {
    ...
    var handleBracketClick = function(e) {
      console.log("Bracket clickt");
      console.log(e.target);
      var $parentDiv = $(e.target).parents('div').eq(0);
      $parentDiv.toggleClass('collapsed');
      // debugger;
    };

    var $bracket = $('<p>');
    $bracket.on('click', handleBracketClick);
...
// ------ Function to be returned on invocation of renderInteractiveJSON ------
  ...
  var displayObject = function(obj, parentKey, $parentDiv, topDist, leftDist) {
    ...
    if (typeof obj != "object") {
      ...
      obj.forEach(function(el, idx) {
        displayObject(el, idx, $childDiv, topDist, leftDist);
        topDist += lineHeight;
      });
...
// TO DO:
// 1. Change positioning to relative for items inside container divs so that they can be collapsed.
// 2. Create line number divs inside of level divs so that they, too, can be collapsed
// 3. Write CSS so that a plus will appear when levels are collapsed
// 4. Add support for Ruby & JS object/hash/array notation parsing
